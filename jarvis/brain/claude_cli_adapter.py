...
def format_cli_system_prompt(request: BrainRequest) -> str:
    ...
    lines = [
        ...
        "available Jarvis tool; never infer volatile machine state from "
...
def apply_claude_system_prompt(
    ...
    the Jarvis tool contract so it is persisted and visible in ``tool_runs``."""
...
def _without_managed_prompt_flags(tokens: Sequence[str]) -> list[str]:
    """Drop host-provided identity/customization flags before adding Jarvis truth."""
...
class ClaudeCliAdapter:
    ...
    def _start_stdout_reader(
        ...
        thread = threading.Thread(target=read, name="jarvis-claude-stdout", daemon=True)
        thread.start()
        return thread
    ...
    def _start_persistent_stderr_reader(self, process: Any) -> threading.Thread | None:
        ...
        thread = threading.Thread(target=read, name="jarvis-claude-stderr", daemon=True)
        thread.start()
        return thread
...
def _format_completed_checkpoint(
    ...
    for tool_call in response.tool_calls:
        ...
        lines.append(f"<jarvis_tool_call>{payload}</jarvis_tool_call>")
    return "\n".join(lines).rstrip() + "\n"
