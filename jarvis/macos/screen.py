...
- Captures are transient artifacts: PNG files under a jarvisd-owned work
...
- `native` — capture via Apple's `/usr/sbin/screencapture` (itself built on
  ...
  subprocess** (`python -m jarvis.macos.screen --ocr <png>`), so an ABI
...
_DEFAULT_FIXTURE_LINES = [
    "JARVIS FAKE SCREEN — D4 smoke fixture",
    "login: ozzy",
...
class NativeScreenReader(ScreenReader):
    ...
    def read_window(self) -> Mapping[str, Any]:
        ...
        from jarvis.macos.ax_backend import frontmost_window_summary
    ...
    def _require_screen_recording(self) -> None:
        if not preflight_screen_capture_access():
            raise ScreenReadError(
                ...
                "jarvisd. See docs/runbooks/SCREEN_RECORDING_TCC.md."
            )
...
def _ocr_in_subprocess(capture_path: Path) -> list[str]:
    ...
    command = [sys.executable, "-m", "jarvis.macos.screen", "--ocr", str(capture_path)]
    try:
        completed = subprocess.run(
            command,
            capture_output=True,
...
def _run_ocr_cli(path: str) -> int:
    """``python -m jarvis.macos.screen --ocr <png>``: OCR one file to JSON."""

    from jarvis.macos.vision_ocr import VisionOCRError, recognize_text
...
def _probe() -> int:
    """Manual TCC/onboarding probe: ``python -m jarvis.macos.screen``.
    ...
    try:
        ...
        if report["screen_recording"]:
            reader = NativeScreenReader(
                work_dir=Path(tempfile.mkdtemp(prefix="jarvis-screen-probe-"))
            )
            ...
            report["hint"] = (
                "Grant Screen Recording to the process hosting jarvisd: "
...
if __name__ == "__main__":
    ...
    # re-imports it as jarvis.macos.screen — two distinct ScreenReadError
    # classes otherwise (the D2 probe lesson). Delegate to the canonical
    # module instance.
    from jarvis.macos import screen as _canonical
