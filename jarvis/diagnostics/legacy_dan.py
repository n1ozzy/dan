...
# jarvis.runtime.supervisor is the daemon-side legacy-conflict detector and
...
from jarvis.runtime.supervisor import LEGACY_LAUNCH_AGENTS, LEGACY_PROCESS_PATTERNS
...
def match_process_lines(lines: Iterable[str]) -> list[str]:
    ...
    tool itself (jarvis.diagnostics) is excluded."""

    matched: list[str] = []
    for line in lines:
        lowered = line.lower()
        if "jarvis.diagnostics" in lowered:
            continue
