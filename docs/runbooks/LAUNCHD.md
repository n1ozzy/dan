...
Manual-only lifecycle for the single official DAN agent. Nothing here is
ever run automatically — no build step, test, or smoke installs or loads
launchd (LAUNCH_SUPERVISION.md §5, FROZEN).

| Concern | Value |
|---------|-------|
| Official label (the only one) | `com.dan.dand` |
| Wrapper installed to | `~/.dan/bin/dand` |
| Plist installed to | `~/Library/LaunchAgents/com.dan.dand.plist` |
| Logs | `~/.dan/logs/dand.{out,err}.log` |
| Database (never touched by these scripts) | `~/.dan/dan.db` |
...
```bash
launchctl print gui/$(id -u)/com.dan.dand | head -20
...
The wrapper prefers `~/.dan/config.toml` when it exists (user-owned
config outside the repo), then falls back to the repo default. An explicit
`DAN_CONFIG` environment variable wins over both.
...
|------|-----------|----------|
| `~/.dan/logs/dand.log` | the Python `dan` logger (all app diagnostics) | **in-process**, automatic |
| `~/.dan/logs/dand.out.log` | launchd `StandardOutPath` (stray stdout) | newsyslog (optional) |
| `~/.dan/logs/dand.err.log` | launchd `StandardErrorPath` (crash tracebacks) | newsyslog (optional) |

**`dand.log`** carries essentially all volume and rotates itself: a
...
`daemon.log_backup_count` rotated files (`dand.log.1` … `.N`). Defaults:
...
**`dand.{out,err}.log`** only receive whatever escapes the logger — stray
...
```
# /etc/newsyslog.d/dand.conf  — substitute your real home path
# logfilename                              owner:group  mode count size  when flags
/Users/<you>/.dan/logs/dand.out.log  <you>:staff  600  5    10240 *    N
/Users/<you>/.dan/logs/dand.err.log  <you>:staff  600  5    10240 *    N
```
...
reloaded (`launchctl kickstart -k gui/$(id -u)/com.dan.dand`). Given how
little lands there, the newsyslog rule is hygiene, not a hard cap — the real
retention control is the in-process rotation of `dand.log` above.
...
file"*. That is why the wrapper and logs live under `~/.dan`. The
installed wrapper is a thin shim that pins `DAN_REPO` and delegates to
...
1. Check `~/.dan/logs/dand.err.log`. A permission / *operation not
...
2. Grant access: System Settings → Privacy & Security → Files and Folders
   ...
   `launchctl kickstart -k gui/$(id -u)/com.dan.dand`.
...
- One label: `com.dan.dand`. Legacy labels are conflicts to report,
  never to adopt.
