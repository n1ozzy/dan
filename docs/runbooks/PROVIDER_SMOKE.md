...
This runbook covers a manual, text-only Claude CLI brain smoke for DAN v4.1.
...
- Do not touch real `~/.dan`.
- Start only one temporary `dand` child process for the smoke.
...
For a successful turn, inspect `python -m dan.cli --config "$CONFIG" events after --id 0 --url "$BASE_URL"` and confirm:
...
The script uses only `claude -p`, temporary DAN paths, and a single child
`dand` process.

## Common Failures

- HTTP client timeout while the provider CLI is still running; use `--timeout 180`.
- Missing Claude auth; run the provider login flow outside DAN.
