...
- nazwę persony oraz niezmienny snapshot rozwiązanej konfiguracji renderu:
  silnik, głos/style, tempo, profil masteringu, DSP i wersje lub sumy assetów;
...
1. zamrożenie i manifest bieżących źródeł, wartości, assetów, baz, procesów,
   plistów, symlinków i materiałów wejściowych wraz z sumami SHA-256;
...
11. migracja wszystkich konsumentów i wyłączników do `dan speak`, `dan queue`
    oraz API daemona;
...
Rollback przed bramą usuwania zatrzymuje `dand`, przywraca backup konfiguracji,
baz i plistów oraz pozwala ponownie uruchomić stary tor. Stare repozytoria i
bazy źródłowe nie są kasowane w tym samym kroku co cutover.
