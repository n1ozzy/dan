...
Backends:
- `ax` — real AXUIElement reads via ctypes (dan/macos/ax_backend.py).
...
_DEFAULT_FIXTURE_APP = {
    "app_name": "FakePad",
    "bundle_id": "com.dan.fakepad",
    "pid": 4242,
}

_DEFAULT_FIXTURE_WINDOW = {
    "app_name": "FakePad",
    "title": "Fake Window — DAN ui_read smoke",
    "elements": [
...
def create_reader(backend: str) -> AccessibilityReader:
    ...
    if normalized == "ax":
        from dan.macos.ax_backend import AXAccessibilityReader
...
def create_actor(backend: str) -> AccessibilityActor:
    ...
    if normalized == "ax":
        from dan.macos.ax_backend import AXAccessibilityActor
...
def _probe() -> int:
    """Manual TCC/onboarding probe: ``python -m dan.macos.accessibility``.
    ...
    try:
        from dan.macos.ax_backend import AXAccessibilityReader, is_process_trusted
        ...
        if report["trusted"]:
            ...
            report["hint"] = (
                "Grant Accessibility to the process hosting dand: "
...
if __name__ == "__main__":
    ...
    # it as dan.macos.accessibility — two distinct AccessibilityError
    # classes, so a local _probe would not catch the backend's errors.
    # Delegate to the canonical module instance.
    from dan.macos import accessibility as _canonical
