"""Parse explicit DAN tool-call blocks from provider text output."""
...
from dan.brain.base import BrainToolCall
...
def parse_tool_call_blocks(provider_output: str) -> ToolCallParseResult:
    """Extract explicit DAN tool-call blocks without executing anything."""
...
def _parse_single_block(payload_text: str, block_index: int) -> tuple[BrainToolCall | None, str | None]:
    ...
    # derived downstream from the registered DAN tool spec by name.
    return (
        BrainToolCall(
            id=call_id,
            name=name.strip(),
