...
2. Mic-side barge-in — real user speech while DAN is speaking or
   generating cancels all three legs (dan/voice/cancellation.py) BEFORE
   the new turn starts.
...
class VoiceTurnGateway:
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="dan-voice-turn"
        )
    ...
    def handle_transcript(self, text: str) -> None:
        ...
        if self._is_speech_active():
            # Mic-side barge-in: the user spoke over DAN and it was NOT
