...
PROVIDER_PRESET: dict[str, dict[str, Any]] = {
    ...
    # Codex CLI intentionally removed as a brain provider (owner decree): DAN
    # runs on Claude Code only. Removed from PROVIDER_PRESET AND from the
    # BrainManager registration/priority (dan/brain/manager.py) so it cannot
...
LOCAL_RUNTIME_PROBES: tuple[dict[str, Any], ...] = (
    {
        ...
        "kind": "Local",
        "commands": ("ollama",),
        "model_env": ("DAN_OLLAMA_MODEL", "OLLAMA_MODEL"),
    },
    {
        "id": "mlx",
        "label": "MLX",
        ...
        "model_env": ("DAN_MLX_MODEL", "MLX_MODEL"),
        "planned": True,
    },
    {
        "id": "llama_cpp_metal",
        ...
        "model_env": ("DAN_LLAMA_CPP_MODEL", "LLAMA_CPP_MODEL"),
        "planned": True,
    },
    {
        "id": "bielik",
        ...
        "kind": "Local model",
        "commands": (),
        "model_env": ("DAN_BIELIK_MODEL", "BIELIK_MODEL"),
        "planned": True,
    },
    {
        "id": "mistral",
        ...
        "kind": "Local model",
        "commands": (),
        "model_env": ("DAN_MISTRAL_MODEL", "MISTRAL_MODEL"),
        "planned": True,
    },
)
...
def _claude_cli_contract(
    *,
    ...
    if not command_found:
        ...
        apply_semantics = "not_apply_capable"
        blocker = "Claude CLI auth is missing; run claude auth login outside DAN."
        ...
        apply_semantics = "not_apply_capable"
        blocker = "Claude CLI permission mode auto is not apply-capable until DAN can prove Claude Code auto-mode eligibility."
...
def _codex_cli_contract(
    *,
    ...
    if not command_found:
        ...
        apply_semantics = "not_apply_capable"
        blocker = "Codex CLI auth is missing; run codex login outside DAN."
...
def _build_voice_capabilities(
    ...
    *,
    ...
    # DAN actually speaks — Polish and English. Whisper accepts ~100 codes
    ...
    *,
    ...
    brain_fields = {
        ...
        "model_source": _preview_field(
            ...
            status="ok" if current_provider.get("model_source") in {"dan_explicit", "claude_default", "codex_default"} else "unknown",
            source="runtime_detected",
            dependencies=["brain_provider.provider"],
        ),
        ...
        "effort_source": _preview_field(
            ...
            status="ok" if current_provider.get("effort_source") in {"dan_explicit", "model_default", "unsupported"} else "unknown",
            source="runtime_detected",
            dependencies=["brain_provider.provider", "brain_provider.model"],
        ),
        "fast": _preview_field(
    ...
    *,
    ...
    if app.config.voice.speak_responses and tts_fields["tts_provider"]["status"] == "missing":
        add(
            ...
            reason="DAN is configured to speak responses, but no usable TTS provider exists.",
