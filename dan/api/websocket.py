"""Read-only WebSocket event stream for the local DAN daemon (RFC 6455).
...
from dan.store.db import close_quietly, connect_db
...
STREAM_SUBPROTOCOL = "dan.v1"
TOKEN_SUBPROTOCOL_PREFIX = "dan-token."
...
def extract_token_candidates(headers: Mapping[str, Any]) -> list[str]:
    ...
    smuggles the token as a ``dan-token.<token>`` subprotocol entry;
        ...
        try:
            ...
            while True:
                ...
                if time.monotonic() - last_ping >= STREAM_PING_INTERVAL_SECONDS:
                    self._send(encode_frame(OP_PING, b"dand"))
                    last_ping = time.monotonic()
